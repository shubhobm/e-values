# quieten SIS: from the package ddpcr
...
get_outputs = function(rho, sigma, k, eq){
    ...
    run_all_methods = function(seed){
        ...
        # initialize
        all_outputs = matrix(0, nrow=6, ncol=p+1)
        
        # run SIS screening
        runtime = system.time({
        quiet(sismodel <- SIS(X1,y, penalty="lasso", perm=TRUE, seed=1e3*seed, iter.max=n))
        })
        sis.inds = sismodel$ix
        p1 = length(sis.inds)
        X = X1[,sis.inds]
        beta.sis = rep(0,p)
        beta.sis[sis.inds] = sismodel$coef.est[-1]
        all_outputs[1,] = c(runtime[3], beta.sis)
        ...
        SSPmat.d = matrix(0, nrow=nboot, ncol=p1+1)
        beta.mat.m = matrix(0 ,nrow=nboot, ncol=p1)
        
        ntau = length(tau_vec)
        beta_hat_list = vector("list",ntau)
        for(ind in 1:ntau){
            # initialize
            beta_hat_list[[ind]] = rep(0,p1)
            tau = tau_vec[ind]
            ...
            SSPmat.d[,p1+1] = depth.Mahalanobis(beta.mat.m, beta.mat.m)
            ...
            which.sel = which(mean.vec[1:p1] < th*mean.vec[p1+1])
        ...
        all_outputs[2,c(1,sis.inds+1)] = c(runtime[3], beta_hat_list[[which.min(bic_vec)]])
        ...
        all_outputs[3,c(1,sis.inds+1)] = c(runtime[3], lasso.obj$beta[-1,which.min(bic.vec)])
        ...
        all_outputs[4,c(1,sis.inds+1)] = c(runtime[3], scad.obj$beta[-1,which.min(bic.vec)])
        
        ## stepwise
        beta.step = rep(0,p1)
        runtime = system.time({
        df = data.frame(cbind(y,X))
        names(df) = c("y",names(df)[-(p1+1)])
        lmod = lm(y~., data=df)
        ...
        all_outputs[5,c(1,sis.inds+1)] = c(runtime[3], beta.step)
        ...
        all_outputs[6,c(1,sis.inds+1)] = c(runtime[3], beta.ko)
        
        # return
        row.names(all_outputs) = c("sis","evalue","lasso","scad","step","ko")
        t(all_outputs)
    }
    ...
    evaluate = function(betamat, beta){
        ...
        metric.mat = mclapply(1:niter, loopfun, mc.cores=50)
        metric.mat = matrix(unlist(metric.mat), ncol=5, byrow=T)
    ...
    nobj = length(out_obj)
    coef.array = array(0, c(p,6,nobj))
    for(it in 1:nobj){
        coef.array[,,it] = out_obj[[it]][-1,]
    }
    
    # compute metrics
    metric.mat = matrix(0, nrow=6, ncol=11)
    row.names(metric.mat) = c("sis","evalue","lasso","scad","step","ko")
