...
# master function
get_outputs_hd_mio = function(rho, sigma, k, eq){
    ...
    run_mio = function(seed){
        ...
        runtime = system.time({
        bs.obj = bs(X, y, intercept=F, k=seq(1,min(p1,29),by=2), time.limit=30)
        ...
        betahat = rep(0,p)
        betahat[sis.inds] = beta.mat[, which.min(bicvals)]
        c(runtime[3], betahat)
    }
    ...
    clusterExport(cl,list('quiet','SIS','bs','run_mio','mvrnorm','n','p','Sigma.X','beta','sigma'))
