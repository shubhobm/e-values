...
# load libraries
# ======================================================================
Required.Packages <- c("R.utils","parallel","ddalpha","ncvreg","knockoff","RcppArmadillo","Rcpp","optparse")
...
# **********************************************************************
option_list = list(
    make_option(c("-n", "--numsamp"), type="double", default=500, help="sample size [default = %default]", metavar="double"),
    make_option(c("-p", "--params"), type="double", default=100, help="params size [default = %default]", metavar="double"),
...
for(rh in 1:nrho){
    ...
    saveRDS(out_list, file=paste0("s1_n",n,"p",p,"s",10*sigma,"b",nboot,"e",as.numeric(eq),".rds")) # save after each rho
    cat("done\n")
}